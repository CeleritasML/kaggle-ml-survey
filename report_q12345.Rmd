...
### The same doesn't necessary go for software engineering or 
...
jobs <- unique(dat$Q5)
for (job in jobs){
    if (job %in% c(swe_side,data_side)){
        print_pie_chart(job)
    }
}
```
...
Is there a significant income gap between men and women for these jobs?
